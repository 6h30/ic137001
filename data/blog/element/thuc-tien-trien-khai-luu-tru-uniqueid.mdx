...
tags: ['revit-api', 'code']
draft: false
summary: Ba phương pháp chính, cấu hình ứng dụng (file JSON), cơ sở dữ liệu (SQLite), và Extensible Storage (trong chính file Revit) để lưu trữ UniqueId của một element.
---

### **Thực Tiễn Lưu Trữ `UniqueId` Trong Ứng Dụng Revit API C\#**
...
- **Ưu điểm:**
  - **Đơn giản và dễ triển khai:** Không yêu cầu thiết lập cơ sở dữ liệu hay các cấu trúc phức tạp.
  - **Dễ đọc và chỉnh sửa:** File JSON có thể được mở và xem bằng trình soạn thảo văn bản.
  - **Di động:** Dễ dàng di chuyển file cấu hình giữa các máy (nếu cần).
- **Nhược điểm:**
  - **Hiệu suất kém với dữ liệu lớn:** Không phù hợp để lưu trữ hàng ngàn `UniqueId` vì mỗi lần đọc/ghi sẽ tải toàn bộ dữ liệu vào bộ nhớ.
  - **Thiếu khả năng truy vấn phức tạp:** Không có các tính năng tìm kiếm, lọc, hoặc sắp xếp mạnh mẽ như cơ sở dữ liệu.
  - **Khó quản lý đồng thời:** Có thể phát sinh vấn đề nếu nhiều ứng dụng hoặc luồng cố gắng ghi vào cùng một file cùng lúc.
  - **Không an toàn:** Dữ liệu không được mã hóa hoặc bảo vệ.
...
- **Ưu điểm:**
  - **Hiệu suất cao với dữ liệu lớn:** Tối ưu cho việc lưu trữ và truy vấn hàng ngàn hoặc hàng triệu bản ghi.
  - **Khả năng truy vấn mạnh mẽ:** Hỗ trợ SQL cho các thao tác tìm kiếm, lọc, sắp xếp dữ liệu phức tạp.
  - **Quản lý dữ liệu tốt hơn:** Cấu trúc dữ liệu có tổ chức, dễ dàng thêm/sửa/xóa bản ghi cụ thể.
  - **Đáng tin cậy:** Cơ chế giao dịch (transaction) đảm bảo tính toàn vẹn của dữ liệu.
- **Nhược điểm:**
  - **Thiết lập phức tạp hơn:** Yêu cầu một số kiến thức về SQL và quản lý cơ sở dữ liệu.
  - **Phụ thuộc vào thư viện:** Cần gói NuGet `System.Data.SQLite.Core`.
  - **File cơ sở dữ liệu riêng biệt:** Mặc dù tự chứa, nhưng vẫn là một file riêng nằm ngoài file Revit, cần được quản lý độc lập.
...
- **Ưu điểm:**
  - **Bền vững và đi kèm file Revit:** Dữ liệu được nhúng trực tiếp vào file `.rvt`, đảm bảo tính nhất quán khi file được chia sẻ, di chuyển hoặc làm việc nhóm (worksharing).
  - **An toàn và bảo mật:** Dữ liệu được bảo vệ bởi Revit (khó truy cập nếu không qua API), và bạn có thể thiết lập cấp độ truy cập (Public, Vendor, Application).
  - **Mối liên hệ chặt chẽ:** Dễ dàng liên kết dữ liệu trực tiếp với các element cụ thể trong mô hình.
  - **Hỗ trợ transaction:** Các thao tác đọc/ghi dữ liệu được thực hiện trong transaction của Revit, đảm bảo tính toàn vẹn.
- **Nhược điểm:**
  - **Phức tạp hơn để triển khai:** Yêu cầu hiểu biết về Schema, Entity, Field và quản lý transaction.
  - **Giới hạn về kích thước dữ liệu:** Không nên lưu trữ quá nhiều dữ liệu lớn vào Extensible Storage vì nó làm tăng kích thước file Revit.
  - **Khó truy vấn toàn cục:** Để tìm tất cả các element có Extensible Storage, bạn phải duyệt qua tất cả các element trong tài liệu, có thể chậm với các mô hình lớn.
  - **Dữ liệu không dễ dàng được "nhìn thấy" bên ngoài:** Khó kiểm tra hoặc chỉnh sửa thủ công nếu không có công cụ đặc biệt.
