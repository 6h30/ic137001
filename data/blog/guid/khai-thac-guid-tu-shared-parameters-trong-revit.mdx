...
Trong môi trường BIM, đặc biệt là với Revit, **Shared Parameters** đóng vai trò cực kỳ quan trọng trong việc chuẩn hóa dữ liệu và đảm bảo khả năng tương tác giữa các dự án và phần tử. Mỗi Shared Parameter được xác định duy nhất bởi một **Global Unique Identifier (GUID)**. Việc truy cập và thao tác với GUID này là chìa khóa để tự động hóa, kiểm soát chất lượng dữ liệu, và phát triển các công cụ mạnh mẽ thông qua Revit API.

Bài viết này chúng ta thực hành 4 phương pháp chính để trích xuất GUID từ Shared Parameters.

---

### 1\. Truy Cập Trực Tiếp GUID Từ Đối Tượng `Parameter`
...
### 2\. Duyệt Qua Các Định Nghĩa Shared Parameter Trong Tệp Shared Parameter (Shared Parameter File)
...
### 3\. Sử Dụng GUID để Truy Cập Parameter (`Element.get_Parameter(Guid)`)
...
### 4\. Sử Dụng RevitLookup (Công Cụ Debugging)
